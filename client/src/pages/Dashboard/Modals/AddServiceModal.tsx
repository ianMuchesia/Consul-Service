...
const AddServiceModal = ({ openModal, handleCloseModal,parentName }: Props) => {
  ...
  useEffect(() => {
    if (isEditMode && service) {
      ...
      if (service.check) {
        ...
        setCheckEntries(newCheckEntries.length ? newCheckEntries : [
          { key: 'HTTP', value: '' },
          { key: 'Interval', value: '10s' },
          { key: 'Timeout', value: '5s' },
        ]);
      ...
      // If creating a new service with prefilled parent name
      setValue('Name', parentName);
    }
  }, [isEditMode, service, setValue, appendTag, parentName]);
  ...
  return (
    <Transition
      appear
      show={openModal.isOpen && (openModal.modalName === "ADD-SERVICE" || openModal.modalName === "EDIT-SERVICE")}
      as={Fragment}
    >
      <Dialog
        as="div"
              ...
              >
                ...
                >
                  <div className="flex flex-col gap-4 max-h-[60vh] overflow-y-auto">
                    ...
                    <div className={`${errors.Port && "has-error"}`}>
                      ...
                      <input
                        {...register("Port", {
                          ...
                          valueAsNumber: true,
                          validate: value => 
                            (!isNaN(value) && value > 0) || "Port must be a positive number"
                        })}
                        type="number"
                    ...
                    {/* Check Fields */}
                    <div className="mt-2">
                      <label className="form-label">Health Check <span className="text-red-500">*</span></label>
                          ...
                          <div key={index} className="flex gap-2 mb-2">
                            ...
                            {index >= 3 && (
                              <button
                                type="button"
                                className="btn btn-sm btn-danger"
                                onClick={() => removeCheckEntry(index)}
                              >
                                Remove
                              </button>
                            )}
